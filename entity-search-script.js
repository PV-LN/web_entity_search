...
$(function ()
{
    ...
    $('#btnSearch').show().click(function () {let searchQuery = `"${$('#firstName').val()}" "${$('#secondName').val()}" "${$('#organisation').val()}" "${$('#location').val()}"`; Search(searchQuery,0);});
    $('#lnkPrev').click(function () { let searchQuery = `"${$('#firstName').val()}" "${$('#secondName').val()}" "${$('#organisation').val()}" "${$('#location').val()}"`;Search(searchQuery,-1); });
    $('#lnkNext').click(function () { let searchQuery = `"${$('#firstName').val()}" "${$('#secondName').val()}" "${$('#organisation').val()}" "${$('#location').val()}"`;Search(searchQuery,1);  });
});
